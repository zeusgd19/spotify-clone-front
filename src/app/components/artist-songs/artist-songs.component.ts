  ...
  playSong(song: Song) {
    ...
    try {
      if(!localStorage.getItem('jwt_token')){
        this.songsService.audio = new Audio(`spotifyclone.vps.webdock.cloud/stream/${song.name}/${song.artist.name}`);
        this.songsService.audio.play();
      } else {
        if(this.auth.userProfile.product != 'premium'){
          this.songsService.audio = new Audio(`spotifyclone.vps.webdock.cloud/stream/${this.artist!.name}/${song.name}`);
